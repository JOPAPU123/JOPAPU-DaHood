...
local Controller = getgenv().JopapuController or {States = {}, Connections = {}}
...
local function startAimLines()
    stopAimLines()
    for _, v in ipairs(Players:GetPlayers()) do
        OnPlayer(v)
    end
    table.insert(VisualConnections, Players.PlayerAdded:Connect(OnPlayer))
end

function Controller.SetAimLines(state)
    Controller.States.AimLines = state
    if state then
        startAimLines()
    else
        stopAimLines()
